...
export interface ContactFormBlockValue {
  formTitle?: string;
  formSubtitle?: string;
...
export function ContactFormGroup({
  ...
  const formTitle = value.formTitle ?? "Send Us a Message";
  const formSubtitle =
    value.formSubtitle ??
    "Fill out the form below and one of our consultants will be in touch shortly.";
  const interestOptions =
    value.interestOptions?.length ? value.interestOptions : defaultInterestOptions;
  const privacyPolicyHref = value.privacyPolicyHref ?? "/privacy-policy";
  const successTitle = value.successTitle ?? "Message Sent!";
